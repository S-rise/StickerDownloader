...
func RewardDailyOnce(update *tgbotapi.Update, reward int) int {
	...
	if limit == "" {
		...
		logger.Info.Printf("%s [RewardDailyOnce] ADD [%d] (new user)", utils.LogUserInfo(update), reward)
		return reward
	}

	limitTimes, _ := strconv.Atoi(limit)
	newTotal := limitTimes + reward
	rdb.Set(ctx, limitKey, newTotal, 24*time.Hour)

	logger.Info.Printf("%s [RewardDailyOnce] ADD [%d] -> [%d]", utils.LogUserInfo(update), reward, newTotal)
